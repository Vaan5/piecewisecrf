...
def train(dataset, resume_path=None):
    ...
    with tf.Graph().as_default():
        ...
        (image, labels_unary, labels_orig, labels_bin_sur,
            labels_bin_above_below, img_name,
            weights, weights_surr, weights_ab) = reader.inputs(dataset,
                                                               shuffle=True,
                                                               num_epochs=FLAGS.max_epochs,
                                                               dataset_partition='train')
        (image_val, labels_unary_val, labels_orig_val,
            labels_bin_sur_val, labels_bin_above_below_val,
            img_name_val, weights_val, weights_surr_val, weights_ab_val) = reader.inputs(dataset,
                                                                                         shuffle=False,
                                                                                         num_epochs=FLAGS.max_epochs,
                                                                                         dataset_partition='validation')
        (image_train, labels_unary_train, labels_orig_train,
            labels_bin_sur_train, labels_bin_above_below_train,
            img_name_train, weights_train, weights_surr_train,
            weights_ab_train) = reader.inputs(dataset,
                                              shuffle=False,
                                              num_epochs=FLAGS.max_epochs,
                                              dataset_partition='train')
        ...
        with tf.variable_scope('model'):
            ...
            # Calculate loss.
            loss = model.loss(unary_log, pairwise_log, pairwise_ab_log, labels_unary,
                              labels_bin_sur, labels_bin_above_below, FLAGS.batch_size,
                              weights, weights_surr, weights_ab)

        with tf.variable_scope('model', reuse=True):
            unary_log_val, pairwise_log_val, pairwise_ab_log_val = model.inference(image_val,
                                                                                   FLAGS.batch_size,
                                                                                   is_training=False)
            # Calculate loss.
            loss_val = model.loss(unary_log_val, pairwise_log_val, pairwise_ab_log_val, labels_unary_val,
                                  labels_bin_sur_val, labels_bin_above_below_val, FLAGS.batch_size, weights_val,
                                  weights_surr_val, weights_ab_val, is_training=False)

        # used for validating performance on train set
        with tf.variable_scope('model', reuse=True):
            unary_log_train, pairwise_log_train, pairwise_ab_log_train = model.inference(image_train,
                                                                                         FLAGS.batch_size,
                                                                                         is_training=False)
            # Calculate loss.
            loss_train = model.loss(unary_log_train, pairwise_log_train, pairwise_ab_log_train, labels_unary_train,
                                    labels_bin_sur_train, labels_bin_above_below_train, FLAGS.batch_size,
                                    weights_train, weights_surr_train, weights_ab_train, is_training=False)
        ...
        for epoch_num in range(1, FLAGS.max_epochs + 1):
            ...
            if FLAGS.evaluate_train_set:
                (train_loss, train_pixacc,
                    train_iou, train_recall,
                    train_precision) = evaluate(sess, unary_log_train,
                                                [(pairwise_log_train, list(zip(indices.FIRST_INDICES_SURR,
                                                                               indices.SECOND_INDICES_SURR))),
                                                 (pairwise_ab_log_train, list(zip(indices.FIRST_INDICES_AB,
            ...
            (val_loss, val_pixacc, val_iou,
                recall, precision) = evaluate(sess, unary_log_val,
                                              [(pairwise_log_val, list(zip(indices.FIRST_INDICES_SURR,
                                                                           indices.SECOND_INDICES_SURR))),
                                               (pairwise_ab_log_val, list(zip(indices.FIRST_INDICES_AB,
                                                                              indices.SECOND_INDICES_AB)))],
                                              loss_val, labels_orig_val, dataset)
