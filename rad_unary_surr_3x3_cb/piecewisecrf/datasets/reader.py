...
def inputs(dataset, shuffle=True, num_epochs=False, dataset_partition='train'):
    ...
    with tf.name_scope('input'):
        ...
        image, labels_unary, labels_bin_sur, img_name, weights, weights_surr = read_and_decode(filename_queue)

        image, labels_unary, labels_bin_sur, img_name, weights, weights_surr = tf.train.batch(
            [image, labels_unary, labels_bin_sur, img_name, weights, weights_surr], batch_size=batch_size, num_threads=2,
            capacity=64)
