...
def neg_log_likelihood(out_unary, out_binary, out_binary_ab, labels_unary, labels_binary, labels_binary_ab, batch_size, weights, weights_surr, weights_ab):
    '''
    ...
    with tf.op_scope([out_unary, out_binary, out_binary_ab,
        ...
        one_hot_labels_binary_ab = tf.one_hot(tf.to_int64(labels_binary_ab), FLAGS.num_classes * FLAGS.num_classes, 1, 0)
        one_hot_labels_binary_ab = tf.reshape(one_hot_labels_binary_ab, [num_neighbours_ab,
                                           FLAGS.num_classes * FLAGS.num_classes])

        out_unary = tf.image.resize_bilinear(out_unary, [FLAGS.img_height, FLAGS.img_width],
                                           name='resize_scores')
        ...
        #pb_ab = tf.mul(tf.to_float(one_hot_labels_binary_ab), log_softmax_binary_ab)

        #loss_val = tf.reduce_sum(-pu) + tf.reduce_sum(-pb) + tf.reduce_sum(-pb_ab)
        loss_val = tf.div(tf.reduce_sum(-pu), tf.to_float(num_examples)) + tf.div(tf.reduce_sum(-pb), tf.to_float(num_neighbours)) + tf.div(tf.reduce_sum(-pb_ab), tf.to_float(num_neighbours_ab))
