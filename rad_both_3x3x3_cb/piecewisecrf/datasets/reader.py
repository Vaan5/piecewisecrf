...
def read_and_decode(filename_queue):
    ...
    return image, labels_unary, labels_bin_sur, labels_bin_above_below, img_name, weights, weights_surr, weights_ab
...
def inputs(dataset, shuffle=True, num_epochs=False, dataset_partition='train'):
    ...
    with tf.name_scope('input'):
        ...
        image, labels_unary, labels_bin_sur, labels_bin_above_below, img_name, weights, weights_surr, weights_ab = read_and_decode(filename_queue)

        image, labels_unary, labels_bin_sur, labels_bin_above_below, img_name, weights, weights_surr, weights_ab = tf.train.batch(
            [image, labels_unary, labels_bin_sur, labels_bin_above_below, img_name, weights, weights_surr, weights_ab], batch_size=batch_size, num_threads=2,
            capacity=64)
