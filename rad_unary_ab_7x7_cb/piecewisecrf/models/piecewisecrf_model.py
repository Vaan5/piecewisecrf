...
def inference(inputs, batch_size, is_training=True):
    ...
    with scopes.arg_scope([slim.ops.conv2d], stddev=0.01, weight_decay=FLAGS.reg_factor, is_training=is_training):
        with scopes.arg_scope([ops.conv2d], is_training=is_training):
            ...
            with scopes.arg_scope([ops.conv2d], batch_norm_params=bn_params):
                ...
                first_indices = tf.gather(pairwise_surr, indices.FIRST_INDICES_AB)
                second_indices = tf.gather(pairwise_surr, indices.SECOND_INDICES_AB)
...
def loss(out_unary, out_binary, labels_unary, labels_binary, batch_size, weights, weights_ab, is_training=True):
    '''
    ...
    '''
    loss_val = losses.neg_log_likelihood(out_unary, out_binary, labels_unary, labels_binary, batch_size, weights, weights_ab)
    all_losses = [loss_val]
