...
def neg_log_likelihood(out_unary, out_binary, labels_unary, labels_binary, batch_size, weights, weights_ab):
    '''
    ...
    with tf.op_scope([out_unary, out_binary, labels_unary, labels_binary], None, 'NegativeLogLikelyhood'):
        ...
        xent_ab = tf.reduce_sum(tf.mul(tf.to_float(one_hot_labels_binary), log_softmax_binary), 1)
        pb = tf.mul(tf.minimum(tf.to_float(100), weights_ab), xent_ab)
        ...
        loss_val = tf.div(tf.reduce_sum(-pu), tf.to_float(num_examples)) + tf.div(tf.reduce_sum(-pb), tf.to_float(num_neighbours))
