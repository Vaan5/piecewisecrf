...
def train(dataset, resume_path=None):
    ...
    with tf.Graph().as_default():
        ...
        image, labels_unary, labels_bin_sur, img_name, weights, weights_ab = reader.inputs(dataset,
                                                                      shuffle=True,
                                                                      num_epochs=FLAGS.max_epochs,
                                                                      dataset_partition='train')
        image_val, labels_unary_val, labels_bin_sur_val, img_name_val, weights_val, weights_ab_val = reader.inputs(dataset,
                                                                                      shuffle=False,
                                                                                      num_epochs=FLAGS.max_epochs,
                                                                                      dataset_partition='validation')
        image_train, labels_unary_train, labels_bin_sur_train, img_name_train, weights_train, weights_ab_train = reader.inputs(
                                                                                    dataset,
                                                                                    shuffle=False,
                                                                                    num_epochs=FLAGS.max_epochs,
                                                                                    dataset_partition='train')
        ...
        with tf.variable_scope('model'):
            ...
            # Calculate loss.
            loss = model.loss(unary_log, pairwise_log, labels_unary, labels_bin_sur, FLAGS.batch_size, weights, weights_ab)
        ...
        with tf.variable_scope('model', reuse=True):
            ...
            loss_val = model.loss(unary_log_val, pairwise_log_val, labels_unary_val,
                                  labels_bin_sur_val, FLAGS.batch_size, weights_val, weights_ab_val, is_training=False)
        ...
        with tf.variable_scope('model', reuse=True):
            ...
            loss_train = model.loss(unary_log_train, pairwise_log_train, labels_unary_train,
                                  labels_bin_sur_train, FLAGS.batch_size, weights_train, weights_ab_train, is_training=False)
